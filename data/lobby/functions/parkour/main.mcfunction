...
#handle checkpoints
execute at @e[type=area_effect_cloud,tag=parkour,tag=checkpoint] align xyz positioned ~-1 ~ ~-1 as @a[dx=2,dy=3,dz=2,gamemode=adventure,scores={pk_checkpoint=1..}] if score @e[type=area_effect_cloud,tag=parkour,tag=checkpoint,distance=..4,sort=nearest,limit=1] pk_checkpoint > @s pk_checkpoint run tag @s add Ryukkun.bolt.nextCP
...
tag @a remove Ryukkun.bolt.nextCP
