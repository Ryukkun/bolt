...
scoreboard players operation @e[type=area_effect_cloud,distance=..0.0001,tag=kh_selected,tag=kh_arrow,sort=nearest,limit=1] gm_id = $id kh_mathTacking

#id the armour stand and area_effect_cloud
scoreboard players operation @s kh_arrowId = $arrows kh_arrowId
scoreboard players operation @e[type=area_effect_cloud,distance=..0.0001,tag=kh_selected,tag=kh_arrow,sort=nearest,limit=1] kh_arrowId = $arrows kh_arrowId

scoreboard players add $arrows kh_arrowId 1

#update tags
tag @e[type=area_effect_cloud,distance=..0.0001,tag=kh_selected,tag=kh_arrow,sort=nearest,limit=1] remove kh_selected
tag @s add setup
tag @s add kh_arrow
