...
execute if score @s gm_team matches 1 as @e[type=item,tag=ice,scores={gm_team=2},distance=..2] run tag @s add failed
execute if score @s gm_team matches 2 as @e[type=item,tag=ice,scores={gm_team=1},distance=..2] run tag @s add failed
